...
class TicTacToe {
	...
	static boolean checkDiagWin() {
		if(board[0][0] !=' ' && board[0][0]==board[1][1] && board[1][1]==board[2][0]
...
abstract class Player{
	...
	boolean isValidMove(int row , int col) {
		if( row>=0 && row<=2 && col>=0 && col<=2) { // checking placing inside the bondier 
			if(TicTacToe.board[row][col]==' ') { //trying to placing inside of it and is empty are 	 	 not
				return true;
			}
		}
		return false;
